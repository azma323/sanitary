...
import { 
  ...
  Users as UsersIcon, X, Wallet, LogOut, Users, Truck, AlertTriangle, Trash2, RefreshCcw
} from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className="min-h-screen flex bg-slate-950 selection:bg-amber-400/30">
      {isSidebarOpen && <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 lg:hidden" onClick={closeSidebar} />}

      <aside className={`fixed inset-y-4 left-4 w-64 bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl z-50 transition-all duration-300 lg:translate-x-0 lg:static lg:block lg:my-4 lg:ml-4 lg:mr-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="h-full flex flex-col p-6">
          <div className="flex items-center justify-between mb-10">
            <Link to="/" onClick={closeSidebar} className="flex items-center justify-start group cursor-pointer">
              <img src="/full-logo.png" alt="Logo" className="w-auto h-16 object-contain group-hover:scale-105 transition-transform duration-300 drop-shadow-lg" />
            </Link>
            <button onClick={closeSidebar} className="lg:hidden text-slate-400 hover:text-white"><X className="w-6 h-6" /></button>
          </div>
          ...
          <nav className="flex-1 space-y-2 overflow-y-auto custom-scrollbar">
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.path} to={item.path} onClick={closeSidebar} className={`flex items-center gap-3 px-5 py-4 rounded-2xl transition-all duration-300 group ${isActive ? 'bg-amber-400 text-slate-950 font-bold shadow-lg shadow-amber-500/10' : 'text-slate-400 hover:bg-slate-800/50 hover:text-amber-400'}`}>
          ...
          <div className="pt-6 border-t border-slate-800/50 space-y-4">
            ...
            <button onClick={onLogout} className="w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-slate-400 hover:bg-rose-500/10 hover:text-rose-500 transition-all duration-300 group border border-transparent hover:border-rose-500/20">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-20 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-30 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/50">
          <div className="flex items-center gap-6">
            <button onClick={() => setIsSidebarOpen(true)} className="p-3 bg-slate-900 border border-slate-800 rounded-2xl text-amber-400 lg:hidden shadow-xl"><Menu className="w-6 h-6" /></button>
            <div className="hidden lg:flex items-center gap-2 bg-slate-900/50 border border-slate-800 px-4 py-2.5 rounded-2xl backdrop-blur-md">
              ...
              <input type="text" placeholder="Global search..." className="bg-transparent border-none outline-none text-sm text-slate-300 w-48 placeholder:text-slate-600" />
            </div>
          </div>

          <div className="flex items-center gap-4">
             <div className="relative">
                <button onClick={() => isSuperAdmin && setIsStoreMenuOpen(!isStoreMenuOpen)} disabled={!isSuperAdmin} className={`flex items-center gap-3 bg-slate-900 border border-slate-800 px-4 py-2.5 rounded-2xl text-sm font-bold transition-all backdrop-blur-md ${isSuperAdmin ? 'hover:border-amber-400/50' : 'cursor-default'}`}>
                ...
                {isStoreMenuOpen && isSuperAdmin && (
                  <>
                    ...
                    <div className="absolute right-0 mt-3 w-64 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl z-20 py-3 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                      ...
                      {stores.map(store => (
                        <button key={store.id} onClick={() => { onStoreChange(store); setIsStoreMenuOpen(false); }} className={`w-full text-left px-5 py-3 text-sm transition-all flex items-center gap-3 ${currentStore.id === store.id ? 'bg-amber-400/10 text-amber-400 font-bold' : 'text-slate-400 hover:bg-slate-800/50 hover:text-white'}`}>
                          <Warehouse className="w-4 h-4 opacity-50" />{store.name}
                        </button>
                      ))}
                    </div>
                  </>
             ...
             <div className="relative">
                <button onClick={() => setIsNotificationOpen(!isNotificationOpen)} className="p-2.5 bg-slate-900 border border-slate-800 rounded-2xl text-slate-400 hover:text-amber-400 transition-colors relative">
                  <Bell className="w-5 h-5" />
                  {lowStockProducts.length > 0 && <span className="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full border-2 border-slate-950 animate-pulse"></span>}
                </button>
                ...
                <AnimatePresence>
                  {isNotificationOpen && (
                    <>
                      ...
                      <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 mt-3 w-80 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl z-50 py-4 overflow-hidden">
                        <div className="px-5 py-2 border-b border-slate-800 mb-2 flex items-center justify-between">
                          ...
                          <span className="bg-rose-500/10 text-rose-500 text-[8px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest">{lowStockProducts.length} Alerts</span>
                        </div>
                            ...
                            lowStockProducts.map(p => (
                              <div key={p.id} className="px-5 py-4 hover:bg-slate-800/50 transition-colors border-b border-slate-800/50 last:border-0">
                                <div className="flex items-start gap-3">
                                  <div className="p-2 bg-rose-500/10 rounded-xl text-rose-500 mt-1"><AlertTriangle className="w-4 h-4" /></div>
                                  <div>
                                    ...
                                    <Link to="/purchases" onClick={() => setIsNotificationOpen(false)} className="text-[9px] text-amber-500 font-black uppercase tracking-widest mt-2 inline-block hover:underline">Restock Now</Link>
                                  </div>
                                </div>
                              </div>
                            ))
